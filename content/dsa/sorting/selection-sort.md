...
选择排序（数组实现）是 **不稳定** 排序算法。根本原因在于每轮用 **一次交换** 把全局最小值放到 `arr[i]`：最小值可能来自很远的位置，而 `arr[i]` 上原有的元素（含相等键）会被换到后面，相对顺序无法保持。

本实现找最小值时用 `arr[j] < arr[min]`，相等时保留较左下标，但这只影响「选哪个最小值」，**不能**保证稳定——问题出在最小值与 `arr[i]` 的 **远距离互换**，而非比较符是否带等号。

以 `[3b, 3a, 1]` 为例（`3b`、`3a` 值均为 3）：

| 轮次 | `i` | `min` | 操作 | 结果 |
| :--- | :--- | :--- | :--- | :--- |
| 1 | `0` | `2`（值 `1`） | 交换 `arr[0]` 的 `3b` 与 `arr[2]` 的 `1` | `[1, 3a, 3b]` |
| 2 | `1` | `1`（值 `3a`） | `i == min`，跳过 | `[1, 3a, 3b]` |

输入时 `3b` 在 `3a` 前面，排序后 `3a` 在 `3b` 前面，相对顺序被破坏。链表实现若用插入代替交换可做到稳定，但数组实现通常只能交换，故不稳定。
