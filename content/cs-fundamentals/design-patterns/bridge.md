...
下文继续使用「电商订单系统」，但问题已经不是 **选哪个支付渠道**（工厂方法），也不是 **把哪个 SDK 接成 `PaymentProcessor`**（适配器）。现在支付后端已有支付宝、微信支付、Stripe，支付请求形态又分 **直接支付**、**分期支付**、**退款请求**，两边都会继续增加。本文关注 **如何把支付请求形态与支付后端拆成两个可独立扩展的维度**。
