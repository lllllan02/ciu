...
- **行为型模式** 关注「对象怎么协作」：当下单前要叠加可配置的风控、合规与渠道差异校验，或退款需按金额在自动批、主管、财务之间分流时，用责任链把处理者串成可组装的链，发送方只交给链头而不认识每一环；当运营后台要改单并撤销、大促批量关单需入队重试、或审计要求回放人工操作时，用命令把「做什么」封装成可执行、可撤销、可排队的对象，调用方只 `Run` 命令而不绑死 `OrderService` 的每一个方法；当仓储导出要扁平化组合明细里的叶子 SKU、报表要分页扫订单而不暴露底层 slice 或游标、或存储从内存迁到 DB 时仍希望调用方遍历方式不变，用迭代器把「怎么逐个访问集合」从聚合内部抽成可替换对象；当结算页上明细、地址、优惠券、配送与支付等多个对等区块任一改动都要联动刷新、却不想组件之间两两引用时，用中介者把交互收到协调中心，同事只通知中介而不直接互调；当运营编辑订单草稿或多步试改后需要回到任意检查点、且不能让历史栈直接读写订单内部字段时，用备忘录由发起人封装快照与恢复，保管者只保存 opaque 的 Memento；当订单支付、发货或取消后要通知邮件、搜索、积分与审计等多个下游、且每增一种反应不应改 OrderService 写逻辑时，用观察者让主题在状态变更后发布领域事件，由订阅者各自处理副作用；当订单在待支付、已支付、已发货等阶段允许的操作不同、且 Pay/Ship/Cancel 等转换规则散落各处 switch 时，用状态模式把每态行为收进独立 State，由上下文委托并在状态内完成迁移；当整单计价、运费或分摊随会员等级、渠道与大促切换不同算法、且不应在 Checkout 各处重复 if 时，用策略模式封装可互换算法并由上下文委托 Calculate；当 App、B 端与跨境站共享预占→计价→支付→落库骨架但前置校验、收款与下单后动作各不同时，用模板方法固定 PlaceOrder 顺序，把可变步骤延迟到钩子实现；当对组合明细树要持续增加 JSON 导出、拣货、税务与审计等只读操作、且不宜让 OrderLine 接口无限变胖时，用访问者通过 Accept 与 Visit 双重分派把新操作封装为独立 Visitor。
...
| 模式 | 描述 |
| :--- | :--- |
| [责任链模式](/cs-fundamentals/design-patterns/chain-of-responsibility) | 使多个对象都有机会处理请求，将处理者连成链并沿链传递，直到有对象处理或全会签通过。 |
| [命令模式](/cs-fundamentals/design-patterns/command) | 将请求封装为对象，从而支持参数化、排队、日志记录以及撤销与重做。 |
| [迭代器模式](/cs-fundamentals/design-patterns/iterator) | 提供顺序访问聚合元素的方法而不暴露内部表示，支持多种遍历策略与分页流式访问。 |
| [中介者模式](/cs-fundamentals/design-patterns/mediator) | 用中介对象封装一组对象之间的交互，使同事不必相互引用，降低网状耦合并集中联动规则。 |
| [备忘录模式](/cs-fundamentals/design-patterns/memento) | 在不破坏封装的前提下捕获并外部保存对象状态，以便之后由发起人将其恢复到先前状态。 |
| [观察者模式](/cs-fundamentals/design-patterns/observer) | 定义一对多依赖，主题状态变更时自动通知所有观察者，使写状态与扇出副作用解耦。 |
| [状态模式](/cs-fundamentals/design-patterns/state) | 允许对象在内部状态改变时改变行为，将各态允许的操作与状态迁移封装在独立 State 中。 |
| [策略模式](/cs-fundamentals/design-patterns/strategy) | 封装一族可互换算法，使它们可独立于客户端替换，消除 Context 内的条件分支。 |
| [模板方法模式](/cs-fundamentals/design-patterns/template-method) | 在模板方法中定义算法骨架，将可变步骤延迟到子类或钩子，复用流程并固定顺序与补偿。 |
| [访问者模式](/cs-fundamentals/design-patterns/visitor) | 在不改变元素类的前提下，将作用于结构上各元素的操作封装为访问者，通过 Accept/Visit 双重分派。 |
