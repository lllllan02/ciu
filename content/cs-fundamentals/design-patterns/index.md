...
- **行为型模式** 关注「对象怎么协作」：当下单前要叠加可配置的风控、合规与渠道差异校验，或退款需按金额在自动批、主管、财务之间分流时，用责任链把处理者串成可组装的链，发送方只交给链头而不认识每一环；当运营后台要改单并撤销、大促批量关单需入队重试、或审计要求回放人工操作时，用命令把「做什么」封装成可执行、可撤销、可排队的对象，调用方只 `Run` 命令而不绑死 `OrderService` 的每一个方法。
