...
**责任链模式** 把 **多个处理者** 连成 **链**，请求从链头传入并沿链传递，直到 **某一环处理完毕** 或 **全部通过**——发送方 **只交给链头**，不必认识每个处理者。
...
下单前校验、退款分级审批等规则拆成独立的处理者，由组装层串成链。`CheckoutFacade` 只需把请求交给链头；新增风控规则或调整审批档位时，改链的组装即可，不必改 Facade 源码。
