...
**组合模式** 把 **单个对象与对象组合** 成树形结构并 **对外提供统一接口**，使客户端 **不必区分叶子与容器**——对整棵树调用同一方法时，容器 **递归转发**，叶子 **执行具体逻辑**。
...
用一句话说：**将对象组合成树形结构以表示「部分-整体」层次，使客户端对单个对象和组合对象的使用具有一致性。**

把订单明细抽象成 `OrderLine`：无论是单品行还是套餐行，都实现同一组方法。结算服务调用 `Total()` 时不必知道底下是 1 个 SKU 还是多层嵌套礼盒——组合节点递归转发，叶子节点执行具体逻辑。
