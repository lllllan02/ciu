...
**外观模式**（Facade）为 **一组子系统** 提供 **统一的高层入口**：客户端只调 `PlaceOrder`，不必逐个认识库存、支付、通知等模块，也 **不必** 知道它们之间的调用顺序——这些协作被收进 Facade 背后。

教材里常归纳成三条 **主要解决的问题**（GoF / Refactoring.Guru 等）：

1. **降低客户端与复杂子系统之间的耦合度** — 客户端只依赖 Facade，不直接依赖 N 个子系统类型。
2. **简化客户端对系统的操作** — 一次 `PlaceOrder(req)` 替代「先调谁、后调谁、失败怎么办」的全套知识。
3. **隐藏子系统内部的协作细节** — 预占、扣款、落库、补偿的顺序 **对客户端不可见**；客户端 **看不到** 也 **不需要 import** 那些 package。

> **「复杂」指什么？** 外观模式里的「复杂子系统」通常指 **子系统数量多、接口多、协作步骤多**（结构上的复杂），**不是** 指某一步算法有多难。三个一行的小函数，若必须按固定顺序调、还要补偿，对客户端来说仍然是「复杂的使用方式」——Facade 简化的是 **怎么用这一组能力**，不是 **组内每一步怎么实现**。

> **和 DRY 什么关系？** 上面三条是 **同一件事的不同说法**：耦合低了，客户端自然 **操作更简单**、**细节被藏起来**；当 HTTP、MQ、CLI **多个入口** 都要下单时，这些收益会具体表现为 **编排只写一遍**（DRY）。单入口时，**降低耦合 + 隐藏协作** 仍然成立，只是「别重复写」这条不那么显眼。

下文继续用「电商订单系统」说明：问题不在于某一步算法难写，而在于 **客户端若要自己串联多个子系统，就既耦合又重复**。
