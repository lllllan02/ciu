...
**装饰器模式** 在 **不改原类** 的前提下 **动态叠加额外行为**：装饰器与被装饰对象 **实现同一接口**，内部 **持有** `inner` 并在转发前后插入逻辑；客户端可按需 **一层层包装**。
...
把会员折扣、优惠券、礼品包装费等做成装饰器：每个装饰器实现 `OrderLine`，持有 `inner OrderLine`，在 `Total()` 里先调内层再调整金额。客户端按需一层层包装，运行时灵活组合，不必为每种组合写子类。
